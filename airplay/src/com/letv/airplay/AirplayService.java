...
public class AirplayService extends Service {
	...
	public void startActivity(int appId, MediaPlayerArg arg) {
		Log.i(TAG, "Airplay startActivity:" + appId);
		...
		if (appId == JniInterface.VIDEO_PLAYER_ID) {
			...
			intent.putExtra("mediaUrl", arg.getMediaUrl());
			intent.putExtra("percent", arg.getPercent());
			intent.putExtra("videoId", arg.getVideoId());
	...
	public class NetStateBroadCastReceiver extends BroadcastReceiver {
		public void onReceive(Context context, Intent intent) {
			...
			Toast.makeText(context, "action"+intent.getAction(), 3000).show();
			...
			if (intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
				...
				if (noConnectivity) {
	        		...
	        		Toast.makeText(context, type+"action"+networkinfo.isConnected(), 3000).show();
	...
	private void releaseResource(){
		...
		String id = "all";
		if (jni.playVideo){
			setMediaPlayerState(id, id.length(), STOP);
	...
	public static native void setMediaPlayerState(String id, int length, int state);
