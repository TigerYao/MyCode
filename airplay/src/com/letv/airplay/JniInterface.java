...
public class JniInterface {
	...
	public class StatusActivity {
		private MediaPlayerActivity beforeActivity;
		private MediaPlayerActivity afterActivity;
		
		StatusActivity(MediaPlayerActivity before, MediaPlayerActivity after){
	...
	public class MediaPlayerState{
		...
		MediaPlayerState(String id, int status){
			videoId = id;
			state = status;
		}
	...
	class MyHandler extends Handler{  
		...
		Log.e(TAG,""+msg.what);
				...
				if (!TextUtils.isEmpty(((MediaPlayerArg)msg.obj).getMediaUrl())){
					if (mediaPlayerActivity == null) {
						...
						if (mediaPlayerActivity.mHandler != null) {
							...
							MediaPlayerActivity.CM cm = mediaPlayerActivity.new CM((MediaPlayerArg)msg.obj);
			...
			case TYPE_PIC_SHOW:
				...
				setShowPicture(true);
				String id = "all";
				AirplayService.setMediaPlayerState(id, id.length(), AirplayService.STOP);
				...
				if (pictureActivity == null){
					Data = null;
					Data = (byte[])msg.obj;
					airplayService.startActivity(PICTURE_SHOW_ID, null);
				}else{
			...
			case TYPE_SET_MEDIA_ACTIVITY:
				if(((StatusActivity)msg.obj).afterActivity == null){
					if (((StatusActivity)msg.obj).beforeActivity == mediaPlayerActivity){
						...
						Log.d(TAG, "mediaPlayerActivity: " + mediaPlayerActivity);
					}
				}else{
					mediaPlayerActivity = ((StatusActivity)msg.obj).afterActivity;
					Log.d(TAG, "mediaPlayerActivity: " + mediaPlayerActivity);
			...
			case TYPE_VIDEO_STATUS:
				if(((MediaPlayerState)msg.obj).getVideoId() == null){
					AirplayService.setMediaPlayerState(null, 0, ((MediaPlayerState)msg.obj).getState());
				}else{
					AirplayService.setMediaPlayerState(((MediaPlayerState)msg.obj).getVideoId(), ((MediaPlayerState)msg.obj).getVideoId().length(), ((MediaPlayerState)msg.obj).getState());
				}
					break;
	...
	public void setMediaActivity(MediaPlayerActivity before, MediaPlayerActivity after) {
	...
	public void setVideoState(String id, int status){
		Log.d(TAG, id + " setVideoState: " + status);
		myHandler.sendMessage(myHandler.obtainMessage(TYPE_VIDEO_STATUS, 0, 0, new MediaPlayerState(id, status)));
	}
	
	public void setAudioState(String id, int status){
		Log.d(TAG, id + " setAudioState: " + status);
		myHandler.sendMessage(myHandler.obtainMessage(TYPE_AUDIO_STATUS, 0, 0, new MediaPlayerState(id, status)));
	}
	...
	public void startMediaPlayer(String mediaUrl, int percent, String id) {
		...
		Log.d(TAG, "startMediaPlayer "+ " mediaUrl: " + mediaUrl + " percent: " + percent + " id: " + id);
