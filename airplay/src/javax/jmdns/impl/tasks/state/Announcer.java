...
public class Announcer extends DNSStateTask {
    ...
    protected void advanceTask() {
    	 synchronized (this.getDns()) {
    		 ...
    		 boolean isAirplay = false;
    		 
    		 this.setTaskState(this.getTaskState().advance());
    		 
    		 //Log.d("JMDNS", "JMDNS Announcer advanceTask [this.state:] " + this.getTaskState());
    		 
             if (this.getDns().isAssociatedWithTask(this, this.getTaskState())) {
            	 //this.setTaskState(this.getTaskState().advance());
            	// Log.d("JMDNS", "JMDNS Announcer advanceTask jmdns self");
                 if (!this.getTaskState().isAnnouncing()) {
                	 
                	 //Log.d("JMDNS", "JMDNS Announcer advanceTask jmdns self startRenewer");
                     this.cancel();
                 ...
                 for (ServiceInfo serviceInfo : this.getDns().getServices().values()) {
                     ...
                     synchronized (info) {
                         if (info.isAssociatedWithTask(this, this.getTaskState())) {
                        	...
                        	// Log.d("JMDNS", "JMDNS Announcer advanceTask jmdns [info.getType]: " + type);
                        	 if(type.contains("airplay") || type.contains("raop")){
            	...
            	// this.setTaskState(this.getTaskState().advance());
            	// Log.d("JMDNS", "JMDNS Announcer advanceTask jmdns serviceinfo");
                 if (!this.getTaskState().isAnnouncing()) {
                	 
                	 if(isAirplay == true){
                		 this.cancel();
                     
                		 this.recoverObjectsState(stateObjects);
                		 try {
							Thread.sleep(2000);
                		 ...
                		 this.getDns().startProber();
                	//	 Log.d("JMDNS", "JMDNS Announcer advanceTask jmdns serviceinfo startProber");
                	 }else{
                         this.cancel();
                         this.getDns().startRenewer(); 
                	 }
